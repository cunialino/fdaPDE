...
# Add error to simulate data
set.seed(5839745)
# data = rbinom(length(DatiEsatti), size = 1, prob = plogis(DatiEsatti)) # + rnorm(length(DatiEsatti), sd = 0.1)
data = DatiEsatti+ rnorm(length(DatiEsatti), sd = 0.1)
observations=matrix(data,nrow(incidence_matrix),length(time_mesh))

# Set smoothing parameter
lambdaS = 10^seq(-4, 0, .5) 
lambdaT = 10^seq(-4, 0, .5)
...
output_CPP<-smooth.FEM.time(observations=observations,
                            ...
                            FEMbasis = FEMbasis, time_mesh = time_mesh, time_locations = time_mesh,
                            lambdaS = lambdaS, lambdaT = lambdaT,
                            BC = BC, max.steps.FPIRLS = 15, threshold.FPIRLS = 10^-6,
                            PDE_parameters = PDE_parameters,
                            FLAG_PARABOLIC = TRUE)#,
                            # family = "binomial")
