...
plot.settings <- function(data, set, filename) {
  TrueMats = NULL
  ParamMats = NULL
  times <- sort(
...
plot.results <- function(sols, set, filename) {
  ...
  for (j in seq(1, 5)) {
    ...
    image(set$xvec, set$yvec, TrueMats[[j]], col = fieldPal, xlab = "",
          ylab = "", zlim = zl, axes = F, cex = textcex)
    contour(set$xvec, set$yvec, TrueMats[[j]], add = TRUE, labcex = labcex,
            zlim = zl, levels = levs, lwd = lwd)
    mtext(paste("t=", round(times[j], digits = 2), sep = ""), side = 2,
          line = 0, las = 2, cex = titlecex)
    if (j == 1) {
      mtext("True Field", font = 2, cex = titlecex)
    ...
    if (whichones[1]) {
      image(set$xvec, set$yvec, Mats[[j]], col = fieldPal, xlab = "", ylab = "",
            zlim = zl, axes = F, cex = textcex)
      contour(set$xvec, set$yvec, Mats[[j]], add = TRUE, labcex = labcex,
              zlim = zl, levels = levs, lwd = lwd)
      if (j == 1) {
    ...
    if (whichones[2]) {
      image(set$xvec, set$yvec, MatsSep[[j]], col = fieldPal, xlab = "",
            ylab = "", zlim = zl, axes = F, cex = textcex)
      contour(set$xvec, set$yvec, MatsSep[[j]], add = TRUE, labcex = labcex,
              zlim = zl, levels = levs, lwd = lwd)
      if (j == 1) {
    ...
    if (whichones[3]) {
      image(set$xvec, set$yvec, TPSMats[[j]], col = fieldPal, xlab = "",
            ylab = "", zlim = zl, axes = F, cex = textcex)
      contour(set$xvec, set$yvec, TPSMats[[j]], add = TRUE, labcex = labcex,
              zlim = zl, levels = levs, lwd = lwd)
      if (j == 1) {
    ...
    if (whichones[4]) {
      image(set$xvec, set$yvec, SOAPMats[[j]], col = fieldPal, xlab = "",
            ylab = "", zlim = zl, axes = F, cex = textcex)
      contour(set$xvec, set$yvec, SOAPMats[[j]], add = TRUE, labcex = labcex,
              zlim = zl, levels = levs, lwd = lwd)
      if (j == 1) {
