...
# Sampling gamma data --------------------------------------------
response <- rgamma(nrow(loc), shape=mu/scale.param, scale=scale.param)


# Fitting data --------------------------------------------
output_CPP_exact <- smooth.FEM(location = loc, observations = as.numeric(response), FEMbasis =FEMbasis, covariates = desmat,  DOF.evaluation = "stochastic",lambda.selection.lossfunction = "GCV",
