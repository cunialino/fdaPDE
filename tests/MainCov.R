...
for (fam in fams) {
  ...
  for (sim in 1:set$NSIM) {
    ...
    betas$GSRtPDE <- rbind(
        betas$GSRtPDE, sims$GSRtPDE$beta[, sims$GSRtPDE$bestlambda[1],
                                         sims$GSRtPDE$bestlambda[2]])
    betas$GSRPDE <- rbind(
        betas$GSRPDE, sims$GSRPDE$beta[, sims$GSRPDE$bestlambda[1],
  ...
  jpeg(filename = paste(
           set$dir_name, "/figures/BPBeta", set$FAMILY, ".jpeg", sep = ""),
       width = 1200, height = 1200, res = 300)
  par(mfrow = c(1, 2), oma = c(1, 1, 1, 0), mar = c(1, 1, 1, 1))
  boxplot(
      list(beta1$TPS, beta1$SOAP, beta1$GSRtPDE, beta1$GSRPDE), names = NULL)
  abline(h = -.2, col = "red")
  boxplot(
      list(beta2$TPS, beta2$SOAP, beta2$GSRtPDE, beta2$GSRPDE), names = NULL)
  abline(h = .3, col = "red")
  dev.off()
  save(list = c("RMSE", "sims", "set", "time", "betas"),
