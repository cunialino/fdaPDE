runsim <- function(set, whichone = c(T, T, T, T), sim) {
  ...
  if (!is.null(set$betas)) {
    ...
    SOAPform <- resp ~
        cov1 + cov2 + te(x, y, t, bs = c("sf", "cr"), k = c(25, 4), d = c(2, 1),
                         xt = list(bnd = set$fsb)) +
        te(x, y, t, bs = c("sw", "cr"), k = c(25, 4), d = c(2, 1),
           xt = list(bnd = set$fsb))
  } else {
    TPSform <- resp ~ te(x, y, t, k = c(30, 5), d = c(2, 1), bs = c("tp", "cr"))
    SOAPform <- resp ~ te(x, y, t, bs = c("sf", "cr"), k = c(25, 4),
                          d = c(2, 1), xt = list(list(bnd = set$fsb, nmax=15), NULL)) +
                       te(x, y, t, bs = c("sw", "cr"), k = c(25, 4),
                          d = c(2, 1), xt = list(list(bnd = set$fsb, nmax=15), NULL))
  }
  ...
  if (whichone[1]) {
    tt <- Sys.time()
    reslist$GSRtPDE <- smooth.FEM.time(
        time_mesh = set$time_mesh, time_locations = set$time_locations,
        locations = set$loc, observations = data, BC = set$BC,
        FEMbasis = set$FEMbasis, covariates = desmat,
