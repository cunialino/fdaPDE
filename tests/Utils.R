runsim <- function(set, whichone = c(T, T, T, T), sim) {
  ...
  desmat <- NULL
  field <- set$f(
    set$space_time_locations[, 2], set$space_time_locations[, 3],
  ...
  if (set$FAMILY == "binomial") {
    data <- rbinom(
      n = nrow(set$space_time_locations), size = 1,
      prob = plogis(field)
    )
  }
  if (set$FAMILY == "gamma") {
    data <- rgamma(
      n = nrow(set$space_time_locations),
  ...
  if (set$FAMILY == "exponential") {
    data <- rexp(n = nrow(set$space_time_locations), )
  }

  mgcvDat <- data.frame(
    "resp" = data, "t" = set$space_time_locations[, 1],
  ...
  if (whichone[1]) {
    ...
    reslist$GSRtPDE <- smooth.FEM.time(
      time_mesh = set$time_mesh,
      time_locations = set$time_locations,
      locations = set$loc, observations =
        data, BC = set$BC, FEMbasis =
        set$FEMbasis, covariates = desmat,
      DOF.evaluation = "stochastic",
      lambda.selection.lossfunction = "GCV",
      lambdaS = set$lambdaS, lambdaT =
        set$lambdaT, max.steps.FPIRLS = set$maxiters,
  ...
  if (whichone[2]) {
    ...
    reslist$GSRPDE <- smooth.FEM.time(
      time_mesh = set$time_mesh,
      time_locations = set$time_locations,
      locations = set$loc, observations = data,
      FEMbasis = set$FEMbasis, covariates =
        desmat, DOF.evaluation =
        "stochastic",
      lambda.selection.lossfunction = "GCV",
      lambdaS = set$lambdaSs, lambdaT =
        set$lambdaTs, max.steps.FPIRLS = 15,
      family = set$FAMILY, mu0 = NULL,
