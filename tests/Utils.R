runsim <- function(set, whichone = c(T, T, T, T), sim) {
    ...
    plotF <- (sim == 1)
    desmat = NULL
    field = set$f(set$space_time_locations[, 2], set$space_time_locations[, 3],
                  set$space_time_locations[, 1], set$FAMILY)
    if (!is.null(set$betas)) {
        ...
        TPSform <- resp ~
            cov1 + cov2 + te(x, y, t, k = c(30, 5), d = c(2, 1), bs = c("tp", "cr"))
    ...
    if (set$FAMILY == "binomial") {
        data = rbinom(n = nrow(set$space_time_locations), size = 1,
                      prob = plogis(field))
    }
    if (set$FAMILY == "gamma") {
        data = rgamma(n = nrow(set$space_time_locations),
                      shape = -1 / field / set$scale, scale = set$scale)
    }
    ...
    mgcvDat = data.frame("resp" = data, "t" = set$space_time_locations[, 1],
                         "x" = set$space_time_locations[, 2],
                         "y" = set$space_time_locations[, 3])
    if (!is.null(set$betas)) {
    ...
    if (plotF)
        plot.settings( data, set,
                      paste(set$dir_name, "/figures/Settings", set$FAMILY, ".pdf", sep = ""))

    reslist = NULL

    if (whichone[1]) {
        tt <- Sys.time()
        reslist$GSRtPDE <- smooth.FEM.time( time_mesh = set$time_mesh, time_locations = set$time_locations, locations = set$loc, observations = data, BC = set$BC, FEMbasis = set$FEMbasis, covariates = desmat, DOF.evaluation = "stochastic", lambda.selection.lossfunction = "GCV", lambdaS = set$lambdaS, lambdaT = set$lambdaT, max.steps.FPIRLS = 15, family = set$FAMILY, mu0 = NULL, scale.param = NULL, FLAG_PARABOLIC = T, threshold.FPIRLS = 10 ^ -6, PDE_parameters = set$PDE_parameters, GCV.inflation.factor = set$inflfac)
        ...
        if (whichone[2]) {
            tt <- Sys.time()
            reslist$GSRPDE <- smooth.FEM.time(
                                              time_mesh = set$time_mesh, time_locations = set$time_locations,
                                              locations = set$loc, observations = data, FEMbasis = set$FEMbasis,
                                              covariates = desmat, DOF.evaluation = "stochastic",
                                              lambda.selection.lossfunction = "GCV", lambdaS = set$lambdaSs,
                                              lambdaT = set$lambdaTs, max.steps.FPIRLS = 15, family = set$FAMILY,
                                              mu0 = NULL, scale.param = NULL, FLAG_PARABOLIC = F,
                                              threshold.FPIRLS = 10 ^ -6, PDE_parameters = set$PDE_parameters,
                                              BC = set$BC)
            reslist$timeGSRPDE = Sys.time() - tt
            print(reslist$timeGSRPDE)
            bl <- reslist$GSRPDE$bestlambda
            if (length(set$lambdaSs) > 1 &&
                (bl[1] == 1 || bl[1] == length(set$lambdaSs)))
                print(paste("Bad lambdaS:", bl[1]))
            if (length(set$lambdaTs) > 1 &&
                (bl[2] == 1 || bl[2] == length(set$lambdaTs)))
                print(paste("Bad lambdaT:", bl[2]))
        } else
            reslist$GSRPDE = NULL
        print("Done")
        if (whichone[3]) {
            ...
            if (set$FAMILY == "gamma" || set$FAMILY == "exponential")
                reslist$TPS <- gam(TPSform, method = "GCV.Cp",
                                   family = Gamma(link = "inverse"), data = mgcvDat)
            else
                reslist$TPS <- gam(
                                   TPSform, method = "GCV.Cp", family = set$FAMILY, data = mgcvDat)
        ...
        if (whichone[4]) {
            ...
            if(nrow(set$loc) == nrow(set$mesh$nodes)){
                idx = NULL
                boundaryidx <- which(set$mesh$nodesmarkers == 1)
                for(j in 1:set$M){
                    idx[[j]] <- boundaryidx + (j-1)*set$FEMbasis$nbasis
                }
                idx <- unlist(idx)
                mgcvDatS <- mgcvDat[-idx, ]
            }
            else{
                mgcvDatS <- mgcvDat
            }
            tt <- Sys.time()
            if (set$FAMILY == "gamma" || set$FAMILY == "exponential")
                reslist$SOAP = gam(SOAPform, knots = set$knots,
                                   family = Gamma(link = "inverse"), data = mgcvDatS)
            else
                reslist$SOAP =
...
eval.rmse <- function(sols, set, covariates = F) {
    ...
    if (!is.null(sols$GSRtPDE))
        gsrpde <- eval.FEM.time(
                                sols$GSRtPDE$fit.FEM.time, space.time.locations = set$evalGrid[, 1:3],
                                lambdaS = sols$GSRtPDE$bestlambda[1],
                                lambdaT = sols$GSRtPDE$bestlambda[2])
    print(Sys.time() - tt)
    if (!is.null(sols$GSRPDE))
        gsrtpde <- eval.FEM.time(
                                 sols$GSRPDE$fit.FEM.time, space.time.locations = set$evalGrid[, 1:3],
    ...
    if (!is.null(sols$TPS))
        tps <- c *
            predict.gam(
                        sols$TPS, newdata = set$evalGrid)  # , block.size = 10, type =
    # "lpmatrix")%*%coef(sols$TPS)

    if (!is.null(sols$SOAP))
        soap <- c *
            predict.gam(
                        sols$SOAP, newdata = set$evalGrid)  # , block.size = 10, type =
