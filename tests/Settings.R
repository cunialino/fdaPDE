...
settings <- function(flagMeshC = T) {
  ...
  if (flagMeshC) {
    ...
    Settings$mesh = create.mesh.2D(nodes = rbind(boundary_nodes, locations),
    ...
    tmp <- cbind(rep(seq(0, 1, length.out = Settings$N), Settings$N),
    ...
    inner_nodes = tmp[-which(tmp[, 1] == 0 |
                             tmp[, 1] == 1 | tmp[, 2] == 0 | tmp[, 2] == 1), ]
    Settings$mesh = create.mesh.2D(nodes = rbind(boundary_nodes, inner_nodes),
                                   order = Settings$basis_order)
    dimeval = 1000
    Settings$evalGrid <- data.frame(
  ...
  Settings$time_locations =
      seq(0, 1, length.out = Settings$m)  # c(0, sort(runif(Settings$m, 0.0001,
                                          # 1))) #
  if (flagMeshC) {
    Settings$loc = cbind(runif(2 * Settings$n, min = -1, max = 4),
    ...
    Settings$lambdaS = 10 ^ seq(-2, 2, 1)
    Settings$lambdaT = 10 ^ seq(-2, 1, 1)

    Settings$lambdaSs = 10 ^ seq(-1, 3, 1)
  ...
  Settings$space_time_locations =
      cbind(rep(Settings$time_locations, each = nrow(Settings$loc)),
  ...
  if (flagMeshC) {
    ...
    Settings$knots =
        data.frame(
            ...
            y = rep(c(-.6, -.3, .3, .6), rep(8, 4)))  # data.frame("x"=Settings$mesh$nodes[which(Settings$mesh$nodesmarkers
                                                      # == 0), 1],
    ...
    Settings$dir_name =
        paste("P", 2 * Settings$basis_order, "_", "N", Settings$N, "_", "M",
              Settings$M, "_", "n", Settings$n, "_", "m", Settings$m, sep = "")
    if (!dir.exists(Settings$dir_name)) {
      dir.create(Settings$dir_name)
      dir.create(paste(Settings$dir_name, "/figures", sep = ""))
    }
    ...
    Settings$nmax = 100
    Settings$knots =
        data.frame("x" = Settings$mesh$nodes[which(Settings$mesh$nodesmarkers ==
                                                   0), 1],
                   "y" = Settings$mesh$nodes[which(Settings$mesh$nodesmarkers ==
                                                   0), 2])
  }
...
is.in.horseshoe <- function(x, y) {
  ...
  if ((x - 3) ^ 2 + (y - r) ^ 2 < (r - r0) ^ 2 & x > 3) return(TRUE)
  if (r0 ^ 2 <= x ^ 2 + y ^ 2 & x ^ 2 + y ^ 2 <= (2 * r - r0) ^ 2 & x <= 0)
    return(TRUE)
  if ((x - 3) ^ 2 + (y + r) ^ 2 < (r - r0) ^ 2 & x > 3) return(TRUE)
  if (abs(y) > r0 & abs(y) < 2 * r - r0 & 0 < x & x <= 3) return(TRUE)
  return(FALSE)
