...
DataProblem<Integrator_noPoly, ORDER, mydim, ndim>::DataProblem(
    SEXP Rdata, SEXP Rorder, SEXP Rfvec, SEXP RheatStep, SEXP RheatIter,
    SEXP Rlambda, SEXP Rnfolds, SEXP Rnsim, SEXP RstepProposals, SEXP Rtol1,
    SEXP Rtol2, SEXP Rprint, SEXP Rsearch, SEXP Rmesh)
    : deData_(Rdata, Rorder, Rfvec, RheatStep, RheatIter, Rlambda, Rnfolds,
              Rnsim, RstepProposals, Rtol1, Rtol2, Rprint, Rsearch),
    ...
    for (auto it = data.begin(); it != data.end();) {
        tri_activated = mesh_.findLocationNaive(
            data[it - data.begin()]);  // cambiare ricerca
        if (tri_activated.getId() == Identifier::NVAL) {
            it = data.erase(it);
            Rprintf(
                "WARNING: an observation is not in the domain. It is removed "
...
void DataProblem<Integrator_noPoly, ORDER, mydim, ndim>::fillPsiQuad() {
    ...
    for (UInt i = 0; i < Integrator_noPoly::NNODES; i++) {
        Eigen::Matrix<Real, Nodes, 1> evaluator =
            reference_eval_point<Nodes, mydim>(Integrator_noPoly::NODES[i]);
...
template <typename Integrator_noPoly, UInt ORDER, UInt mydim, UInt ndim>
Real DataProblem<Integrator_noPoly, ORDER, mydim,
                 ndim>::FEintegrate_exponential(const VectorXr& g) const {
    using EigenMap2WEIGHTS =
        Eigen::Map<const Eigen::Matrix<Real, Integrator_noPoly::NNODES, 1> >;
    constexpr UInt Nodes = mydim == 2 ? 3 * ORDER : 6 * ORDER - 2;
    ...
    for (UInt triangle = 0; triangle < mesh_.num_elements(); triangle++) {
        ...
        total_sum +=
            expg.dot(EigenMap2WEIGHTS(&Integrator_noPoly::WEIGHTS[0])) *
            tri_activated.getMeasure();
    }
...
template <typename Integrator_noPoly, UInt ORDER, UInt mydim, UInt ndim>
SpMat DataProblem<Integrator_noPoly, ORDER, mydim, ndim>::computePsi(
    const std::vector<UInt>& indices) const {
    UInt nnodes = mesh_.num_nodes();
    UInt nlocations = indices.size();
    SpMat psi(nlocations, nnodes);

    Real eps = 2.2204e-016, tolerance = 100 * eps;
    ...
    for (auto it = indices.cbegin(); it != indices.cend(); it++) {
        ...
        if (tri_activated.getId() == Identifier::NVAL) {
            Rprintf(
                "WARNING: the following observation is not in the domain\n");
            ...
            for (UInt node = 0; node < Nodes; ++node) {
                ...
                evaluator = tri_activated.evaluate_point(deData_.getDatum(*it),
                                                         coefficients);
                triplets.emplace_back(it - indices.cbegin(),
                                      tri_activated[node].getId(), evaluator);
            }
        }
    }
