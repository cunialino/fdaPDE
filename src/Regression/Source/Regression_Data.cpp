...
RegressionData::RegressionData(Real* locations, UInt n_locations, UInt ndim,
                               VectorXr& observations, UInt order,
                               MatrixXr& covariates, VectorXr& WeightsMatrix,
                               std::vector<UInt>& bc_indices,
                               std::vector<Real>& bc_values,
                               MatrixXi& incidenceMatrix, bool arealDataAvg,
                               UInt search)
    : locations_(locations, n_locations, ndim),
      observations_(observations),
      arealDataAvg_(arealDataAvg),
      WeightsMatrix_(WeightsMatrix),
      order_(order),
      bc_values_(bc_values),
      bc_indices_(bc_indices),
      covariates_(covariates),
      incidenceMatrix_(incidenceMatrix),
...
RegressionData::RegressionData(SEXP Rlocations, SEXP RbaryLocations,
                               SEXP Robservations, SEXP Rorder,
                               SEXP Rcovariates, SEXP RBCIndices,
                               SEXP RBCValues, SEXP RincidenceMatrix,
                               SEXP RarealDataAvg, SEXP Rsearch)
    ...
    bc_indices_.assign(INTEGER(RBCIndices),
                       INTEGER(RBCIndices) + length_indexes);
...
RegressionData::RegressionData(SEXP Rlocations, SEXP RbaryLocations,
                               SEXP Rtime_locations, SEXP Robservations,
                               SEXP Rorder, SEXP Rcovariates, SEXP RBCIndices,
                               SEXP RBCValues, SEXP RincidenceMatrix,
...
RegressionDataElliptic::RegressionDataElliptic(
...
RegressionDataEllipticSpaceVarying::RegressionDataEllipticSpaceVarying(
    SEXP Rlocations, SEXP RbaryLocations, SEXP Robservations, SEXP Rorder,
...
void RegressionData::setObservationsTime(SEXP Robservations) {
    ...
    UInt count = 0;
    locations_by_nodes_ =
        (locations_.nrows() == 0 && nRegions_ == 0) ? true : false;
...
void RegressionData::setBaryLocations(SEXP RbaryLocations) {
    ...
    if (TYPEOF(RbaryLocations) !=
        0) {  //TYPEOF(RbaryLocations) == 0 means SEXPTYPE is NILSXP (Description is NULL)
        UInt* id_ = INTEGER(VECTOR_ELT(RbaryLocations, 1));
        Real* bary_ = REAL(VECTOR_ELT(RbaryLocations, 2));

        UInt n_ = INTEGER(
            Rf_getAttrib(VECTOR_ELT(RbaryLocations, 2), R_DimSymbol))[0];
        UInt p_ = INTEGER(
            Rf_getAttrib(VECTOR_ELT(RbaryLocations, 2),
                         R_DimSymbol))[1];  //barycenter column dimension
...
void RegressionData::setIncidenceMatrix(SEXP RincidenceMatrix) {
    ...
    for (auto i = 0; i < nRegions_; ++i) {
        for (auto j = 0; j < p; ++j) {
            incidenceMatrix_(i, j) =
                INTEGER(RincidenceMatrix)[i + nRegions_ * j];
        }
    }
}
