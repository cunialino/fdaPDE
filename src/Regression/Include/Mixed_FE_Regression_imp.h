...
template <typename InputHandler>
void MixedFERegressionBase<
    InputHandler>::addDirichletBC()  //adds boundary conditions to all
{
    UInt id1, id3;
...
template <typename InputHandler>
void MixedFERegressionBase<
    InputHandler>::addDirichletBC_matrix()  //adds boundary conditions to all
{
    UInt id1, id3;
...
void MixedFERegressionBase<InputHandler>::addNA() {
    const std::vector<UInt>* observations_na =
        regressionData_.getObservationsNA();

    for (UInt id : *observations_na) {
        for (UInt j = 0; j < psi_.cols(); ++j) {
            if (psi_.coeff(id, j) != 0) psi_.coeffRef(id, j) = 0;
        }
    }
    psi_.pruned();
    psi_.makeCompressed();
...
void MixedFERegressionBase<InputHandler>::setPsi(
    ...
    UInt nlocations = regressionData_.isSpaceTime()
                          ? regressionData_.getNumberofSpaceObservations()
                          : regressionData_.getNumberofObservations();
    psi_.resize(nlocations, nnodes);  // Resize the matrix

    // Optimized strategies according to the presence of locations
    if (regressionData_.isLocationsByNodes() &
        !regressionData_
             .isLocationsByBarycenter())  // .Pointwise data -- no barycenters
    {
        ...
        if (!regressionData_.isSpaceTime())  // Just spatial case
        {
            ...
            const std::vector<UInt>* k =
                regressionData_.getObservationsIndices();
            UInt k_size = k->size();
    ...
    } else if (regressionData_.isLocationsByBarycenter() &&
               (regressionData_.getNumberOfRegions() ==
                0))  // Pointwise data -- by barycenter
    {
        ...
        for (UInt i = 0; i < nlocations;
             i++) {  // Update Psi looping on all locations
            // We already know the id of the element containing the point
            tri_activated = mesh_.getElement(regressionData_.getElementId(i));

            if (tri_activated.getId() ==
                Identifier::NVAL) {  // Invald id --> error
                Rprintf(
                    "ERROR: Point %d is not in the domain, remove point and "
                    "re-perform smoothing\n",
                    i + 1);
            } else {  // tri_activated.getId() found, it's action might be felt a priori by all the psi of the element, one for each node
                for (
                    UInt node = 0; node < Nodes;
                    ++node) {  // Loop on all the nodes of the found element and update the related entries of Psi
                    evaluator = regressionData_.getBarycenter(
                        i, node);  // We already know the value to add
        ...
        constexpr UInt Nodes = mydim == 2 ? 3 * ORDER : 6 * ORDER - 2;
        Element<Nodes, mydim, ndim> tri_activated;  // Dummy for element search
        Eigen::Matrix<Real, Nodes, 1>
            coefficients;  // Dummy for point evaluation
        Real evaluator;    // Dummy for evaluation storage
        ...
        for (UInt i = 0; i < nlocations;
            ...
            if (regressionData_.getSearch() == 1) {  // Use Naive search
                tri_activated = mesh_.findLocationNaive(
                    regressionData_.template getLocations<ndim>(i));
            } else if (regressionData_.getSearch() ==
                       2) {  // Use Tree search (default)
                tri_activated = mesh_.findLocationTree(
                    regressionData_.template getLocations<ndim>(i));
            }

            // Search the element containing the point
            if (tri_activated.getId() == Identifier::NVAL) {  // If not found
                Rprintf(
                    "ERROR: Point %d is not in the domain, remove point and "
                    "re-perform smoothing\n",
                    i + 1);
                ...
                element_ids_(i) =
                    tri_activated
                        .getId();  // Save the id of the ELEMENT containing the location
                ...
                for (
                    ...
                    evaluator = tri_activated.evaluate_point(
                        regressionData_.template getLocations<ndim>(i),
                        coefficients);
                    // Save barycenter information
                    barycenters_(i, node) = tri_activated.getBaryCoordinates(
                        regressionData_.template getLocations<ndim>(i))[node];
                    // Insert the value in the column given by the GLOBAL indexing of the evaluated NODE
                    psi_.insert(i, tri_activated[node].getId()) = evaluator;
                }
            }
    ...
    } else  // Areal data
    {
        ...
        {  // Loop to update by rows Psi matrix
            for (UInt k = 0; k < nnodes; k++) tab[k] = 0;  //Iniialize at 0
            for (UInt j = 0; j < mesh_.num_elements(); j++) {
                if ((*(regressionData_.getIncidenceMatrix()))(i, j) ==
                    1)  // Element j is in region i
                {       // Location is related to that mesh element
                    Element<Nodes, mydim, ndim> tri =
                        mesh_.getElement(j);  // Identify the element
                    for (
                        UInt k = 0; k < Nodes;
                        k++) {  // Add contribution of the area to right location
                        tab[tri[k].getId()] +=
                            tri.integrate(Eigen::Matrix<Real, Nodes, 1>::Unit(
                                k));  // integral over tri of psi_k
                    }
                }
            }
            for (UInt k = 0; k < nnodes;
                 k++) {           // Loop update Psi matrix i-th row
...
void MixedFERegressionBase<InputHandler>::setH(void) {
    ...
    H_ =
        ...
        (Wt * (*Wp))
            .ldlt()
            .solve(
                Wt);  // using cholesky LDLT decomposition for computing hat matrix
...
template <typename InputHandler>
template <UInt ORDER, UInt mydim, UInt ndim>
void MixedFERegressionBase<InputHandler>::setA(
    const MeshHandler<ORDER, mydim, ndim>& mesh_) {
    UInt nRegions =
        regressionData_
            .getNumberOfRegions();  // Number of regions for areal partition
    // If the problem is temporal, m stores the number of temporal nodes, else is defaulted as 1
    UInt m = regressionData_.isSpaceTime()
                 ? regressionData_.getNumberofTimeObservations()
                 : 1;
...
template <typename InputHandler>
MatrixXr MixedFERegressionBase<InputHandler>::LeftMultiplybyQ(
    const MatrixXr& u) {
...
void MixedFERegressionBase<InputHandler>::buildSpaceTimeMatrices() {
    ...
    if (regressionData_.getFlagParabolic()) {  // Parabolic case
        MixedFDRegression<InputHandler> FiniteDifference(mesh_time_,
                                                         regressionData_);
    ...
    if (this->isSpaceVarying) {  // otherwie no forcing term needed
        VectorXr forcingTerm = rhs_ft_correction_;  // Store old data
        rhs_ft_correction_.resize(M_ * N_);         // New size
        for (UInt i = 0; i < N_;
             i++)  // Update forcing term (i.e. repeat identically M_ times)
        {
            for (UInt j = 0; j < M_; j++) {
...
template <typename InputHandler>
void MixedFERegressionBase<InputHandler>::getRightHandData(
    VectorXr& rightHandData) {
    UInt nnodes = N_ * M_;  // Total number of spatio-temporal nodes
    UInt nlocations =
        regressionData_.getNumberofObservations();  // Count number of locations
    const VectorXr* obsp =
        regressionData_.getObservations();  // Get the value of the observations

    rightHandData = VectorXr::Zero(nnodes);  // Initialize the rhd at 0
    if (regressionData_.getCovariates()->rows() == 0) {  // No covariates case
        if (regressionData_.isLocationsByNodes() &&
            !regressionData_
                 .isSpaceTime()) {  // Regresionbyodes + just space --> Psi^t*z [simplified Psi] (or Psi^t*Q*z in GAM)
        ...
        } else {  // Areal data, no optimization allowed --> Psi^t*A*z [or Psi^t*A*P*z in GAM]
            // LeftMultiplybyQ does nothing since Q==I unless in GAM [where multipliction by Q also involves P (weight matrix)]
            rightHandData =
...
template <typename InputHandler>
void MixedFERegressionBase<InputHandler>::buildMatrixNoCov(
    const SpMat& NWblock, const SpMat& SWblock, const SpMat& SEblock) {
    ...
    std::vector<coeff> tripletAll;
    tripletAll.reserve(NWblock.nonZeros() + 2 * SWblock.nonZeros() +
                       SEblock.nonZeros());
    ...
    for (UInt k = 0; k < SEblock.outerSize(); ++k)
        for (SpMat::InnerIterator it(SEblock, k); it; ++it) {
            tripletAll.push_back(
                coeff(it.row() + nnodes, it.col() + nnodes, it.value()));
        }
    for (UInt k = 0; k < SWblock.outerSize(); ++k)
        for (SpMat::InnerIterator it(SWblock, k); it; ++it) {
            tripletAll.push_back(
                coeff(it.col(), it.row() + nnodes, it.value()));
        }
    for (UInt k = 0; k < SWblock.outerSize(); ++k)
        for (SpMat::InnerIterator it(SWblock, k); it; ++it) {
            tripletAll.push_back(
                coeff(it.row() + nnodes, it.col(), it.value()));
        }
...
void MixedFERegressionBase<InputHandler>::system_factorize() {
    UInt nnodes = N_ * M_;  // Note that is only space M_=1
    const VectorXr* P =
        regressionData_.getWeightsMatrix();  // Matrix of weights for GAM

    // First phase: Factorization of matrixNoCov
    matrixNoCovdec_.compute(matrixNoCov_);

    if (regressionData_.getCovariates()->rows() !=
        0) {  // Needed only if there are covariates, else we can stop before
        ...
        if (regressionData_.getNumberOfRegions() == 0) {  // pointwise data
            ...
            if (P->size() == 0)
                ...
                U_.topRows(nnodes) =
                    psi_.transpose() * A_.asDiagonal() * P->asDiagonal() * W;
        }
...
MatrixXr MixedFERegressionBase<InputHandler>::system_solve(
...
template <typename InputHandler>
void MixedFERegressionBase<InputHandler>::computeDegreesOfFreedom(
    UInt output_indexS, UInt output_indexT, Real lambdaS, Real lambdaT) {
    std::string GCVmethod = optimizationData_.get_DOF_evaluation();
    switch (GCVmethod == "exact") {
        case 1:
            computeDegreesOfFreedomExact(output_indexS, output_indexT, lambdaS,
                                         lambdaT);
            break;
        case 0:
            computeDegreesOfFreedomStochastic(output_indexS, output_indexT,
                                              lambdaS, lambdaT);
            break;
    }
}
...
template <typename InputHandler>
void MixedFERegressionBase<InputHandler>::computeGeneralizedCrossValidation(
    UInt output_indexS, UInt output_indexT, Real lambdaS, Real lambdaT) {
    VectorXr dataHat;
    const VectorXr* z = regressionData_.getObservations();
    if (regressionData_.getCovariates()->rows() ==
        0)  //Data estimated from the model
        dataHat =
            psi_ * _solution(output_indexS, output_indexT).topRows(psi_.cols());
    else
        dataHat =
            *z - LeftMultiplybyQ(*z) +
            LeftMultiplybyQ(
                psi_ *
                _solution(output_indexS, output_indexT).topRows(psi_.cols()));
    UInt n = dataHat.rows();
    if (regressionData_.isSpaceTime()) {
        const std::vector<UInt>* observations_na =
            regressionData_.getObservationsNA();
    ...
    //! GCV computation
    _GCV(output_indexS, output_indexT) =
        (n / ((n - optimizationData_.get_tuning() *
                       ((this->getDOF())(output_indexS, output_indexT))) *
              (n - optimizationData_.get_tuning() *
                       ((this->getDOF())(output_indexS, output_indexT))))) *
        (*z - dataHat).dot(*z - dataHat);
    if (_GCV(output_indexS, output_indexT) <
        optimizationData_.get_best_value()) {
...
template <typename InputHandler>
void MixedFERegressionBase<InputHandler>::computeDegreesOfFreedomExact(
    UInt output_indexS, UInt output_indexT, Real lambdaS, Real lambdaT) {
...
template <typename InputHandler>
void MixedFERegressionBase<InputHandler>::computeDegreesOfFreedomStochastic(
    UInt output_indexS, UInt output_indexT, Real lambdaS, Real lambdaT) {
    UInt nnodes = N_ * M_;
    UInt nlocations = regressionData_.getNumberofObservations();
    ...
    if (regressionData_.getNumberOfRegions() == 0) {
        ...
        b.topRows(nnodes) =
            psi_.transpose() * A_.asDiagonal() * LeftMultiplybyQ(u);
    }
...
void MixedFERegressionBase<InputHandler>::preapply(
    EOExpr<A> oper, const ForcingTerm& u,
    const MeshHandler<ORDER, mydim, ndim>& mesh_) {
...
template <typename InputHandler>
void MixedFERegressionBase<InputHandler>::buildSystemMatrix(Real lambdaS,
                                                            Real lambdaT) {
    this->R0_lambda =
        (-lambdaS) * R0_;  // build the SouthEast block of the matrix
    ...
    if (regressionData_.isSpaceTime() && !regressionData_.getFlagParabolic()) {
        this->buildMatrixNoCov(this->DMat_ + lambdaT * Ptk_, R1_lambda,
                               R0_lambda);
    } else {
...
MatrixXr MixedFERegressionBase<InputHandler>::apply_to_b(const MatrixXr& b) {
    ...
    if (lambda_ != last_lambda) {
        ...
        if (regressionData_.getDirichletIndices()->size() >
            0)  // if areal data NO BOUNDARY CONDITIONS
        {
...
MatrixXv MixedFERegressionBase<InputHandler>::apply(void) {
    ...
    const VectorXr* obsp =
        regressionData_.getObservations();  // Get observations
    ...
    for (UInt s = 0; s < sizeLambdaS; ++s) {
        for (UInt t = 0; t < sizeLambdaT; ++t) {
            Real lambdaS;
            if (!regressionData_.isSpaceTime() &&
                !isGAMData)  //at the moment only space is implemented
            {
            ...
            if (isGAMData || regressionData_.isSpaceTime() ||
                optimizationData_.get_current_lambdaS() !=
                    optimizationData_.get_last_lS_used()) {
                if (!regressionData_.isSpaceTime()) {
            ...
            if (regressionData_.isSpaceTime() &&
                regressionData_.getFlagParabolic()) {
                for (UInt i = 0; i < regressionData_.getInitialValues()->rows();
                     i++) {
                    _rightHandSide(nnodes + i) -=
                        lambdaS * rhs_ic_correction_(i);
                }
            }
            ...
            if (optimizationData_.get_loss_function() == "GCV" &&
                (!isGAMData && regressionData_.isSpaceTime())) {
    ...
    if (!(isGAMData || regressionData_.isSpaceTime()) &&
        optimizationData_.get_current_lambdaS() !=
            optimizationData_.get_last_lS_used()) {
        optimizationData_.set_last_lS_used(
            optimizationData_.get_current_lambdaS());
    }
...
template <>
class MixedFERegression<RegressionData>
    : public MixedFERegressionBase<RegressionData> {
   public:
    MixedFERegression(const RegressionData& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_)
        : MixedFERegressionBase<RegressionData>(regressionData,
                                                optimizationData, nnodes_){};
    MixedFERegression(const std::vector<Real>& mesh_time,
                      const RegressionData& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_,
                      UInt spline_degree)
        : MixedFERegressionBase<RegressionData>(mesh_time, regressionData,
                                                optimizationData, nnodes_,
                                                spline_degree){};
    ...
    void preapply(const MeshHandler<ORDER, mydim, ndim>& mesh) {
        ...
        MixedFERegressionBase<RegressionData>::preapply(stiff, ForcingTerm(),
                                                        mesh);
    }
};

template <>
class MixedFERegression<RegressionDataElliptic>
    : public MixedFERegressionBase<RegressionDataElliptic> {
   public:
    MixedFERegression(const RegressionDataElliptic& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_)
        : MixedFERegressionBase<RegressionDataElliptic>(
              regressionData, optimizationData, nnodes_){};
    MixedFERegression(const std::vector<Real>& mesh_time,
                      const RegressionDataElliptic& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_,
                      UInt spline_degree)
        : MixedFERegressionBase<RegressionDataElliptic>(
              mesh_time, regressionData, optimizationData, nnodes_,
              spline_degree){};
    ...
    void preapply(const MeshHandler<ORDER, mydim, ndim>& mesh) {
        ...
        const Diffusion<PDEParameterOptions::Constant>& K =
            this->regressionData_.getK();
        const Advection<PDEParameterOptions::Constant>& b =
            this->regressionData_.getBeta();

        MixedFERegressionBase<RegressionDataElliptic>::preapply(
            c * mass + stiff[K] + b.dot(grad), ForcingTerm(), mesh);
    }
};

template <>
class MixedFERegression<RegressionDataEllipticSpaceVarying>
    : public MixedFERegressionBase<RegressionDataEllipticSpaceVarying> {
   public:
    MixedFERegression(const RegressionDataEllipticSpaceVarying& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_)
        : MixedFERegressionBase<RegressionDataEllipticSpaceVarying>(
              regressionData, optimizationData, nnodes_){};
    MixedFERegression(const std::vector<Real>& mesh_time,
                      const RegressionDataEllipticSpaceVarying& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_,
                      UInt spline_degree)
        : MixedFERegressionBase<RegressionDataEllipticSpaceVarying>(
              mesh_time, regressionData, optimizationData, nnodes_,
    ...
    void preapply(const MeshHandler<ORDER, mydim, ndim>& mesh) {
        ...
        const Diffusion<PDEParameterOptions::SpaceVarying>& K =
            this->regressionData_.getK();
        const Advection<PDEParameterOptions::SpaceVarying>& b =
            this->regressionData_.getBeta();
        const ForcingTerm& u = this->regressionData_.getU();

        this->isSpaceVarying = TRUE;

        MixedFERegressionBase<RegressionDataEllipticSpaceVarying>::preapply(
...
void MixedSplineRegression<InputHandler>::setPhi(void) {
    ...
    for (UInt i = 0; i < m; ++i) {
        for (UInt j = 0; j < M; ++j) {
            value = spline.BasisFunction(
                j, this->regressionData_.getTimeLocations()[i]);
            if (value != 0) {
...
// -- GAM PART --
template <>
class MixedFERegression<GAMDataLaplace>
    : public MixedFERegressionBase<RegressionData> {
   public:
    MixedFERegression(const RegressionData& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_)
    ...
    MixedFERegression(const std::vector<Real>& mesh_time,
                      const RegressionData& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_)
        : MixedFERegressionBase<RegressionData>(
              mesh_time, regressionData, optimizationData, nnodes_,
              MixedSplineRegression<RegressionData>::SPLINE_DEGREE){};
...
template <>
class MixedFERegression<GAMDataElliptic>
    : public MixedFERegressionBase<RegressionDataElliptic> {
   public:
    MixedFERegression(const RegressionDataElliptic& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_)
    ...
    MixedFERegression(const std::vector<Real>& mesh_time,
        ...
        : MixedFERegressionBase<RegressionDataElliptic>(
              mesh_time, regressionData, optimizationData, nnodes_,
              MixedSplineRegression<RegressionDataElliptic>::SPLINE_DEGREE){};
...
template <>
class MixedFERegression<GAMDataEllipticSpaceVarying>
    : public MixedFERegressionBase<RegressionDataEllipticSpaceVarying> {
   public:
    MixedFERegression(const RegressionDataEllipticSpaceVarying& regressionData,
                      OptimizationData& optimizationData, UInt nnodes_)
    ...
    MixedFERegression(const std::vector<Real>& mesh_time,
        ...
        : MixedFERegressionBase<RegressionDataEllipticSpaceVarying>(
              mesh_time, regressionData, optimizationData, nnodes_,
              MixedSplineRegression<
                  RegressionDataEllipticSpaceVarying>::SPLINE_DEGREE){};
