...
FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::FPIRLS_Base(
    const MeshHandler<ORDER, mydim, ndim>& mesh, InputHandler& inputData,
    OptimizationData& optimizationData, VectorXr mu0, bool scale_parameter_flag,
    Real scale_param)
    : mesh_(mesh),
      inputData_(inputData),
      optimizationData_(optimizationData),
      regression_(inputData, optimizationData, mesh.num_nodes()),
      mu_(mu0),
      scale_parameter_flag_(scale_parameter_flag),
      _scale_param(scale_param) {
...
FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::FPIRLS_Base(
    ...
    const std::vector<Real> mesh_time, InputHandler& inputData,
    OptimizationData& optimizationData, VectorXr mu0, bool scale_parameter_flag,
    Real scale_param)
    : mesh_(mesh),
      mesh_time_(mesh_time),
      inputData_(inputData),
      optimizationData_(optimizationData),
      regression_(mesh_time, inputData, optimizationData, mesh.num_nodes()),
      mu_(mu0),
      scale_parameter_flag_(scale_parameter_flag),
      _scale_param(scale_param) {
    //initialization of mu, current_J_values and past_J_values.  for(UInt j=0; j< inputData.getLambdaS().size() ; j++){
...
// FPIRLS_base methods
template <typename InputHandler, UInt ORDER, UInt mydim, UInt ndim>
void FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::apply(
    const ForcingTerm& u) {
  ...
  */
    ...
    _GCV.resize(LambdaS_len,
                LambdaT_len);  //If GCV is not computed the vector stores -1
    _GCV.setConstant(-1);
    ...
    for (UInt i = 0; i < LambdaS_len;
         i++) {  //for-cycle for each spatial penalization (lambdaS).
        for (UInt j = 0; j < LambdaT_len; j++) {
            current_J_values[0] =
                past_J_values[0] + 2 * inputData_.get_treshold();
            current_J_values[1] =
                past_J_values[1] + 2 * inputData_.get_treshold();
            this->optimizationData_.setCurrentLambda(
                i, j);  // set right lambda for the current iteration.
            ...
            while (stopping_criterion(i, j)) {
                ...
                this->inputData_.updatePseudodata(pseudoObservations_,
                                                  WeightsMatrix_);
                update_solution(i, j);
        ...
        */
            if (this->optimizationData_.get_loss_function() ==
                "GCV") {  // compute GCV if it is required
                compute_GCV(i, j);
            }
...
template <typename InputHandler, UInt ORDER, UInt mydim, UInt ndim>
void FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::update_solution(
    ...
    _fn_hat(lambdaS_index, lambdaT_index) =
        (*Psi) * _solution(lambdaS_index, lambdaT_index).topRows(Psi->cols());
}
...
template <typename InputHandler, UInt ORDER, UInt mydim, UInt ndim>
void FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::compute_mu(
    UInt& lambdaS_index, UInt& lambdaT_index) {
    //compute mu as mu_i = g-1( w_ii*beta + fn_hat)

    VectorXr W_beta =
        VectorXr::Zero(mu_.size());  // initialize the vector w_ii*beta

    if (inputData_.getCovariates()->rows() > 0)
        W_beta = (*(inputData_.getCovariates())) *
                 _beta_hat(lambdaS_index, lambdaT_index);
...
template <typename InputHandler, UInt ORDER, UInt mydim, UInt ndim>
bool FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::stopping_criterion(
    UInt& lambdaS_index, UInt& lambdaT_index) {
    // return true if the f-PIRLS has to perform another iteration, false if it has to be stopped

    bool do_stop_by_iteration =
        false;  // Do I need to stop becouse n_it > n_max?
    bool do_stop_by_treshold =
        false;  // Do I need to stop becouse |J(k) - J(k+1)| < treshold?
    ...
    if (n_iterations(lambdaS_index, lambdaT_index) > inputData_.get_maxiter()) {
        ...
        std::cout << "For lambda_index=( " << lambdaS_index << ", "
                  << lambdaT_index << ") max iterations reached " << std::endl;
    }

    if (n_iterations(lambdaS_index, lambdaT_index) > 1) {
        if (abs(past_J_values[0] + past_J_values[1] - current_J_values[0] -
                current_J_values[1]) < inputData_.get_treshold()) {
            do_stop_by_treshold = true;
        }
    }

    return !(do_stop_by_iteration || do_stop_by_treshold ||
             do_stop_by_exception);
}

template <typename InputHandler, UInt ORDER, UInt mydim, UInt ndim>
std::array<Real, 2> FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::compute_J(
    UInt& lambdaS_index, UInt& lambdaT_index) {
...
template <typename InputHandler, UInt ORDER, UInt mydim, UInt ndim>
void FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::compute_GCV(
    UInt& lambdaS_index, UInt& lambdaT_index) {
    if (optimizationData_.get_DOF_evaluation() !=
        "not_required")  //in this case surely we have already the dofs
    {                    // is DOF_matrix to be computed?
        regression_.computeDegreesOfFreedom(
            0, 0, (*optimizationData_.get_LambdaS_vector())[lambdaS_index],
            (*optimizationData_.get_LambdaT_vector())[lambdaT_index]);
        _dof(lambdaS_index, lambdaT_index) = regression_.getDOF()(0, 0);
    } else
        _dof(lambdaS_index, lambdaT_index) =
            regression_.getDOF()(lambdaS_index, lambdaT_index);
...
void FPIRLS_Base<InputHandler, ORDER, mydim, ndim>::compute_variance_est(
    ...
    if (this->scale_parameter_flag_ &&
        ...
        phi = (this->scale_parameter_flag_)
                  ? this->current_J_values[0] /
                        (n_obs - this->_dof(lambdaS_index, lambdaT_index))
                  : _scale_param;
        for (UInt j = 0; j < this->mu_.size(); j++) {
            _variance_estimates(lambdaS_index, lambdaT_index) +=
                phi * this->var_function(this->mu_[j]) / this->mu_[j];
        }
...
void FPIRLS<GAMDataEllipticSpaceVarying, ORDER, mydim, ndim>::apply() {
    ...
    FPIRLS_Base<GAMDataEllipticSpaceVarying, ORDER, mydim, ndim>::apply(
        this->inputData_.getU());
}
#endif
