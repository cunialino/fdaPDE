...
SEXP GAM_skeleton(InputHandler &GAMData, OptimizationData &optimizationData,
                  SEXP Rmesh, SEXP Rmu0, std::string family, SEXP RscaleParam) {
    ...
    std::unique_ptr<FPIRLS<InputHandler, ORDER, mydim, ndim>> fpirls =
        FPIRLSfactory<InputHandler, ORDER, mydim, ndim>::createFPIRLSsolver(
            family, mesh, GAMData, optimizationData, mu0, scale_parameter);
    ...
    SET_VECTOR_ELT(
        result, 0,
        Rf_allocMatrix(REALSXP, solution(0).size(), solution.size()));
    ...
    SET_VECTOR_ELT(result, 4,
                   Rf_allocMatrix(REALSXP, beta(0).size(), beta.size()));
