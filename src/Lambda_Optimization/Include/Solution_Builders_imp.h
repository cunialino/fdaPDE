...
SEXP Solution_Builders::build_solution_plain_regression(
    ...
    const InputHandler &regressionData) {
    // ---- Preparation ----
    ...
    SET_VECTOR_ELT(result, 0,
                   Rf_allocMatrix(REALSXP, solution.rows(), solution.cols()));
    ...
    SET_VECTOR_ELT(
        result, 1,
        Rf_allocMatrix(REALSXP, output.z_hat.rows(), output.z_hat.cols()));
