...
class FPIRLSfactory
{
	...
	static std::unique_ptr<FPIRLS<InputHandler, Integrator, ORDER,  mydim,  ndim>> createFPIRLSsolver(const std::string &family, const MeshHandler<ORDER,mydim,ndim>& mesh, const std::vector<Real> mesh_time, InputHandler& inputData, VectorXr& mu0, Real scale_parameter)
	{
		//initial checks: m0 must be initialized correctly for the different distributions 
		...
		if(family=="binomial"){
		    return make_unique<FPIRLS_Bernoulli<InputHandler, Integrator, ORDER,  mydim, ndim>>(mesh, mesh_time, inputData, mu0);
		}else if(family=="poisson"){
		    return make_unique<FPIRLS_Poisson<InputHandler, Integrator, ORDER,  mydim, ndim>>(mesh, mesh_time, inputData, mu0);
		}else if(family=="exponential"){
		    return make_unique<FPIRLS_Exponential<InputHandler, Integrator, ORDER,  mydim, ndim>>(mesh, mesh_time, inputData,mu0);
		}else if(family=="gamma"){
