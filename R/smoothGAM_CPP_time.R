CPP_smooth.GAM.FEM.time <- function(
                                    locations, time_locations, observations, FEMbasis, time_mesh,
                                    covariates = NULL, ndim, mydim, BC = NULL, incidence_matrix = NULL,
                                    areal.data.avg = TRUE, FLAG_MASS, FLAG_PARABOLIC, IC, FAMILY, mu0 = NULL,
                                    max.steps.FPIRLS = 15, scale.param = NULL, threshold.FPIRLS = 0.0002020,
    ...
    if (nrow(IC) == 0 && FLAG_PARABOLIC) {
        ...
        if (ICsol[[4]] == 0) {
            ...
            ICsol <- .Call(
                           ...
                           BC$BC_indices, BC$BC_values, incidence_matrix, areal.data.avg, FAMILY,
                           max.steps.FPIRLS, threshold.FPIRLS, mu0, scale.param, search,
                           as.integer(c(0, 1, 1)), lambdaSIC, DOF.stochastic.realizations,
                           DOF.stochastic.seed, DOF.matrix_IC, GCV.inflation.factor,
                           lambda.optimization.tolerance, PACKAGE = "fdaPDE")
            ...
            if (ICsol[[4]] == length(lambdaSIC) - 1) {
                ...
                ICsol <- .Call(
                               ...
                               FAMILY, max.steps.FPIRLS, threshold.FPIRLS, mu0, scale.param,
                               search, as.integer(c(0, 1, 1)), lambdaSIC,
        ...
        ICsol =
            list(IC.FEM = FEM(ICsol[[1]][1:nrow(FEMbasis$mesh$nodes), ], FEMbasis),
    ...
    BC$BC_indices = rep((0:(M - 1)) * nrow(FEMbasis$mesh$nodes),
                        each = length(BC$BC_indices)) + rep(BC$BC_indices, M)
    ...
    bigsol <- .Call(
                    ...
                    FLAG_MASS, FLAG_PARABOLIC, IC, FAMILY, max.steps.FPIRLS, threshold.FPIRLS,
...
CPP_smooth.GAM.FEM.PDE.time <- function(
                                        locations, time_locations, observations, FEMbasis, time_mesh,
                                        covariates = NULL, PDE_parameters, ndim, mydim, BC = NULL,
                                        incidence_matrix = NULL, areal.data.avg = TRUE, FLAG_MASS, FLAG_PARABOLIC,
                                        IC, FAMILY, mu0 = NULL, max.steps.FPIRLS = 15, scale.param = NULL,
    ...
    bigsol <- .Call(
                    ...
                    BC$BC_values, incidence_matrix, areal.data.avg, FLAG_MASS, FLAG_PARABOLIC,
...
CPP_smooth.GAM.FEM.PDE.sv.time <- function(
                                           locations, time_locations, observations, FEMbasis, time_mesh,
                                           covariates = NULL, PDE_parameters, ndim, mydim, BC = NULL,
                                           incidence_matrix = NULL, areal.data.avg = TRUE, FLAG_MASS, FLAG_PARABOLIC,
                                           IC, FAMILY, mu0 = NULL, max.steps.FPIRLS = 15, scale.param = NULL,
    ...
    points_eval =
        matrix(CPP_get_evaluations_points(
                                          mesh = FEMbasis$mesh, order = FEMbasis$order), ncol = 2)
    ...
    if (nrow(IC) == 0 && FLAG_PARABOLIC) {
        ...
        ICsol <- .Call(
                       ...
                       areal.data.avg, search, as.integer(c(0, 1, 1)), lambdaSIC,
                       DOF.stochastic.realizations, DOF.stochastic.seed, DOF.matrix_IC,
                       GCV.inflation.factor, lambda.optimization.tolerance, PACKAGE = "fdaPDE")
        ...
        if (ICsol[[6]] == 1) {
            ...
            ICsol <- .Call(
                           ...
                           PDE_param_eval$u, covariatesIC, BC$BC_indices, BC$BC_values,
                           incidence_matrix, areal.data.avg, search, as.integer(c(0, 1, 1)),
                           lambdaSIC, DOF.stochastic.realizations, DOF.stochastic.seed,
                           DOF.matrix_IC, GCV.inflation.factor, lambda.optimization.tolerance,
                           PACKAGE = "fdaPDE")
        } else {
